...
<div class="p-rl30 p-tb20">
    <div class="row">
        <div class="col-xs-12">
            <h1 class="page-title"><?= lang('settings_social_media_edit_account') ?></h1>
        </div>
    </div>
<!--     <div class="row">
        <div class="col-xs-12">
...
<div class="main_block">
    ...
    <form action="<?php echo site_url('connect/engagement_settings/edit/'.$token->id);?>" method="post">
        <div class="row custom-form edit_profile">
            ...
            <?php endif; ?>
            <?php if($token->type == 'pinterest') : ?>
                <div class="col-sm-6">
                    <div class="choose_account account_facebook">
                        <h3 class="choose_account_title">
                            <i class="fa fa-<?= $token->type ?>"></i> <?= lang('social_board', [ucfirst($token->type)]) ?>
                        </h3>
                        <ul class="choose_account_list">
                                ...
                                <li class="choose_account_list_item">
                                    <label class="cb-radio w-100">
                                        <input
                                            type="radio"
                                            name="board_group"
                                            <?= ($board['id'] == $selected_board_id) ? ' checked="checked"' : '' ?>
                                            value="<?= $board['id'] ?>"
                                            >
                                        <div class="dTable">
                                            <div class="dRow">
                                                <div class="dCell">
                                                    <h3 class="choose_account_list_name"><?= $board['name'] ?></h3>
                                                </div>
                                            </div>
                                        </div>
                                    </label>
                ...
                <div class="col-sm-<?= ($token->type == 'facebook') ? '6' : '12' ?>">
                    <div class="choose_account account_<?= $token->type ?>">
                        ...
                        <?php foreach($available_configs as $available_config) : ?>
                            ...
                            ?>
                            <?php if($available_config['type'] == 'input') : ?>
                                <div class="form-group  col-sm-2">
                                    <p class="text_color"><?= lang($available_config['key']) ?></p>
                                    <select class="form-control" name="config[<?= $available_config['key'] ?>]">
                                        <?php foreach ($locations as $woeid => $place): ?>
                                            <option value="<?php echo $woeid; ?>" <?php echo $available_config['value']==$woeid?'selected':''; ?>> <?php echo $place; ?></option>
                                        <?php endforeach; ?> 
                                    </select>
                                </div>
                            <?php elseif($available_config['type'] == 'text') : ?>
                            ...
                            <?php else: ?>
                                <?php if($available_config['key'] != 'auto_favourite' && $available_config['key'] != 'auto_retweet' && $available_config['key'] != 'auto_follow' && $available_config['key'] != 'smart_mention'): ?>
                                ...
                                <div class="row <?= ($available_config['value']) ? '' : 'is-hidden' ?>" id="<?= $available_config['key'] ?>-config">
                                    <div class="col-xs-12">
                                        <?php if($available_config['key'] == 'smart_engage'):?>
                                            <?php foreach($available_configs as $_available_config) : ?>     
                                                <?php if($_available_config['key'] == 'auto_favourite' || $_available_config['key'] == 'auto_retweet' || $_available_config['key'] == 'auto_follow' || $_available_config['key'] == 'smart_mention'): ?>                                                
                                                    <label class="cb-checkbox w-100 m-l20">
                                                        <input
                                                            type="checkbox"
                                                            name="config[<?= $_available_config['key'] ?>]"
                                                            <?= ($_available_config['value']) ? 'checked="checked"' : '' ?>
                                                        >
                                                        <?= lang($_available_config['key']) ?>
                                                    </label>                                                    
                                                <?php endif; ?>
                                            <?php endforeach; ?>                                       
                                        <?php endif; ?>                                     
