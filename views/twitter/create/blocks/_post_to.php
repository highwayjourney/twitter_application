...
foreach($inactiveSocials as $key => $inactiveSocial) {   
    if(in_array($inactiveSocial, $activeSocials) || $inactiveSocial == 'youtube') {
        unset($inactiveSocials[$key]);
    }
}
//d($inactiveSocials);
$_socials = $socials;
foreach ($_socials as $key => $value) {
    if($value == 'youtube' || $value == 'facebook'){
        unset($_socials[$key]);
    }
}
?>
<p class="text_color strong-size"><?= lang('post_to') ?>
    <span class="custom-form is-relative top-5 p-l10">
        <?php foreach($_socials as $social) : ?>
