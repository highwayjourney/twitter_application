...
<table class="responsive-table admin-users">
    ...
    <?php foreach($users as $user): ?>
        <?php 
        if(!empty($user->getActivePlan())){
            $activePlan = empty($user->getActivePlan()->to_array()['id'])?'e':$user->getActivePlan()->to_array()['id']; 
        }
        $is_admin = $user->id === $c_user->id; 
        ?>
        ...
        <tr>
            ...
            <td data-th="subscription">
                <select class="plan-selection">
                    <option>Select Plan</option>
                    <?php foreach ($plans as $key => $value): ?>
                        <option data-userid="<?php echo $user->id; ?>"  <?php echo $activePlan == $key?'selected':''; ?> value="<?php echo $key; ?>" ><?php echo $value; ?></option>
                    <?php endforeach; ?>
                </select>
            </td>
            <td data-th="<?= lang('action') ?>">
                <ul>
                    ...
                    <?php if ( ! $is_admin): ?>
                        ...
                        <li>
                            <a class="delete-user" href="<?php echo site_url('admin/admin_users/password/' . $user->id); ?>"> Edit Password</a>
                        </li>                        
                    <?php endif; ?>
                </ul>
            </td>
