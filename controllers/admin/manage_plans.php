    ...
    public function specialinvite()
    {
        ...
        if ($email && $plan->id && $specialInvite->id) {
            ...
            if ($success = ($sender->sendSpecialInviteMail($params))) {
                $this->addFlash('Invite Sent -->' .site_url('auth/register/'.$planId.'/'.$inviteCode), 'success');
            } else {
                $this->addFlash(lang('invite_error'));
            }

            echo json_encode(array('success' => $success, 'message' => site_url('auth/register/'.$planId.'/'.$inviteCode)));
        }
    }
}