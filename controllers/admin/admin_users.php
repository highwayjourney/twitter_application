...
class Admin_users extends Admin_Controller {
    ...
    public function index() {
        ...
        $plan =  new Plan;
        $_plans = $plan->getActualPlans(TRUE, TRUE);
        ...
        CssJs::getInst()
            ...
            ->add_js(array(
                ...
                'controller/admin/change_subscription.js'
            ))
    ...
    public function change_plan() {       
        ...
        $_subscription = $subscription->getUserSubscriptions($post['user_id']);
        if(empty($_subscription)){
            $user = new User($post['user_id']);
            $subscriber = $this->get('core.subscriber');
            $subscriber->setUser($user);
            $plan = new Plan((int)$post['plan_id']);
            ...
            $subscription = new Subscription();
            $_subscription = $subscription->getUserSubscriptions($post['plan_id']);
            //$_subscription->addPayment($payment_id);            
        } else {
            $_subscription->modify($post['plan_id']);
        }
        echo "Plan Succesfully Changed";
        // if($_subscription->modify($post['plan_id'])){
        //     echo "Plan Succesfully Changed";
        // }
