...
class Collaboration extends MY_Controller {
    ...
    {
        CssJs::getInst()->add_js('controller/settings/collaborationn.js');
        ...
        foreach ($users as $_user) {
            $_users[] = new user($_user->user_id);
        }
        //ddd($_users[0]->to_array());
        $this->template->set('users', $_users);
        ...
        foreach ($emails as $email) {
            ...
            if ($email) {
                ...
                if ($success = ($result && $sender->sendInviteCollaboratorMail($params))) {
                    ...
                    echo json_encode(array('success' => $success, 'code' => site_url('auth/invite/'.$result['code'])));
                } else {
                    $this->addFlash(lang('invite_error'));
                    echo json_encode(array('success' => $success, 'code' => null));
                }
            }
        }
    }
