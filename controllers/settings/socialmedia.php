...
class SocialMedia extends MY_Controller {
    ...
    public function index() {
        ...
        unset($type_tokens['instagram']);
        unset($type_tokens['google']);
        unset($type_tokens['pinterest']);
        unset($type_tokens['linkedin']);
        //unset($type_tokens['instagram']);
    ...
    public function facebook() {
        try {
            ...
            $redirect_url = $facebook_socializer->getRedirectUrl();
            ...
            if($_SERVER['REMOTE_ADDR'] == '190.206.248.7'){
                $this->addFlash($e->getMessage(), 'error');
            } else {
                $this->addFlash('We are sorry but Facebook Login Interface is under maintenance at the moment', 'error');
            }
            redirect('settings/socialmedia');
        }
    }
    ...
    public function twitter() {
        try {
           ...
           // $this->addFlash($e->getMessage());
            $this->addFlash('Twitter Oauth failed, please  be sure that your twitter API settings are correctly setup');
            redirect('settings/socialmedia');
        }
    }
    ...
    public function instagram() {
        ...
        if(!empty($post['username']) && !empty($post['password'])){
            try {
                ...
                $instagram_socializer->add_new_account($this->profile->id, $username, $password);
                $this->addFlash(lang('connected_success'), 'success');
                redirect(site_url('settings/socialmedia'));
            } catch (Exception $e) {
                $this->addFlash($e->getMessage());
    ...
    /**
     * Used to get access url from Pinterest
     * Use Socializer Library
     * 
     * @access public
     * @return void
    ...
    public function pinterest() {
        try {
            if(Social_group::hasSocialAccountByType($this->profile->id, 'pinterest')) {
                $this->addFlash(lang('already_has_account_error', ['Pinterest']), 'error');
                redirect('settings/socialmedia');
            }
            $this->load->library('Socializer/socializer');
            /* @var Socializer_Linkedin $linkedin_socializer */
            $pinterest_socializer = Socializer::factory('Pinterest', $this->c_user->id);
            
            $redirect_uri = $pinterest_socializer->get_access();
            redirect($redirect_uri);
        } catch (Exception $e) {
            $this->addFlash($e->getMessage());
            redirect('settings/socialmedia');
    ...
    /**
     * Used to connect user to Pinterest account
     * Use Socializer Library
     * Add new record to 'Access tokens' and redirect to settings/socialmedia page
     *
     * @access public
    ...
    public function pinterest_callback() {
        try {
            $this->load->library('Socializer/socializer');
            /* @var Socializer_Linkedin $linkedin_socializer */
            $pinterest = Socializer::factory('Pinterest', $this->c_user->id);
            $code = $_GET["code"];
            $pinterest->add_new_account($this->profile->id, $code);
            $this->addFlash(lang('connected_success'), 'success');
            //redirect(site_url('settings/socialmedia'));
            //redirect(site_url('settings/socialmedia/edit_account/'.$token->id));
        } catch (Exception $e) {
            $this->addFlash($e->getMessage());
            redirect('settings/socialmedia');
        }
