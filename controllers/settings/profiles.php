...
class Profiles extends MY_Controller {
    ...
    public function delete_group($id) {
        try {
            ...
            foreach ($group->access_token->get() as  $access_token) {
                foreach ($access_token->twitter_retweet->get() as $twitter_retweet) {
                    $twitter_retweet->delete();
                }
                foreach ($access_token->twitter_follower->get() as $twitter_follower) {
                    $twitter_follower->delete();
                }
                foreach ($access_token->twitter_mention->get() as $twitter_mention) {
                    $twitter_mention->delete();
                }
                foreach ($access_token->twitter_favourite->get() as $twitter_favourite) {
                    $twitter_favourite->delete();
                }                                               
                $access_token->delete();
            }
            $campaigns = new campaigns;
            foreach ($campaigns->where('profile_id', $this->profile->id)->get() as $campaign) {
                $campaign->delete();
            }             
            $group->delete();
