...
class Create extends MY_Controller
{
    ...
    public function __construct()
    {
        ...
        CssJs::getInst()->c_js('twitter/create', 'app');
        CssJs::getInst()->c_js('twitter/create', 'social_sources');
        CssJs::getInst()->c_js('twitter/create', 'giphy');
        CssJs::getInst()->c_js('twitter/create', 'facebook');
        CssJs::getInst()->c_js('twitter/create', 'youtube');
    }
    ...
    public function uploadImageDesignerFile($mime= 'png') {
        if ($this->template->is_ajax()) {
            ...
            $nameImage = time() .'.'. $mime;
            move_uploaded_file($_FILES['files']['tmp_name'], $urlUploadImages . $nameImage); 
    ...
    {
        redirect('twitter/create/update');
    }
...
//            }
        ...
        if ($this->template->is_ajax()) {
            ...
            if ($user_rss->result_count() > 0) {
                echo $this->template->block(
                    '_suggested_links',
                    'twitter/create/blocks/_custom_rss',
                    array('rss' => $user_rss)
                );
            } else {
        ...
        if ($this->template->is_ajax()) {
            ...
            if (isset($post['link'])) {
                ...
                $html = $this->template->block(
                    '_rss_feed',
                    'twitter/create/blocks/_rss_feed',
    ...
    public function getRssByFeedId($feedId)
    {
            ...
            if ($feedId) {
                ...
                $html = $this->template->block(
                    '_rss_feed',
                    'twitter/create/blocks/_rss_feeder',
    ...
    private function _attach_update_scripts() {
        ...
        CssJs::getInst()->c_js('twitter/create', 'post_attachment');
