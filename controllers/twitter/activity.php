...
//        ),
    ...
    public function index() {
        redirect('twitter/activity/facebook');
    }
    ...
    public function facebook() {
        ...
        CssJs::getInst()->c_js('twitter/activity', 'facebook');
            ...
            if(isset($post['message']) && $post_id != null) {
                try {
                    ...
                    $result['html'] = $this->template->block(
                        '_comment',
                        'twitter/activity/blocks/_one_facebook_comment',
            ...
            if(isset($post['graph_url']) && isset($post['token_id'])) {
                try {
                    ...
                    if(count($page_feed['feed']['data']) > 0) {
                        ...
                        $html = $this->template->block('_facebook_feed', 'twitter/activity/blocks/_facebook_feed',
                            array(
    ...
    private function _get_tweets_html( $type,  $page_number, $access_token_id ) {
        ...
        if(is_array($user_tweets) && empty($errorMessage)) {
            ...
            $tweets_html = $this->load->view('twitter/activity/blocks/_tweets_feed', $block_data, true);
        } else {
    ...
    public function instagram() {
        ...
        CssJs::getInst()->c_js('twitter/activity', 'instagram');
                ...
                if( $mediaId != null) {
                    ...
                    if(!empty($comments->data)) {
                            foreach ( $comments->data as $_comment ) {
                                $html .= $this->template->block(
                                    '_comment',
                                    'twitter/activity/blocks/_one_instagram_comment',
    ...
    private function _get_instagram_html($data) {
        ...
        if(count($user_images) > 0 && !isset($user_images->errors)) {
            ...
            $instagram_html = $this->load->view('twitter/activity/blocks/_instagram_feed', $block_data, true);
        } else {
    ...
    public function google() {
        ...
        CssJs::getInst()->c_js('twitter/activity', 'google');
    ...
    protected function googleActivitiesWrapHtml($items)
    {
        ...
        return  $this->load->view(
            'twitter/activity/blocks/_google_feed', array(
                'activities' => $items,
                'radar' => $this->radar
            ),
            true
                ...
                if( $activityId != null) {
                    ...
                    if( isset($comments['items']) ) {
                            ...
                            foreach ( $comments['items']as $_comment ) {
                                $html .= $this->template->block(
                                    '_comment',
                                    'twitter/activity/blocks/_one_google_comment',
    ...
    public function linkedin()  {
        ...
        CssJs::getInst()->c_js('twitter/activity', 'linkedin');
        $this->template->render();
    }
    ...
    private function _get_linkedin_html($data) {
        ...
        if(count($data) > 0 && !isset($data->errors)) {
            ...
            $block_data['radar'] = $this->radar;
            $linkedin_html = $this->load->view('twitter/activity/blocks/_linkedin_feed', $block_data, true);
            return $linkedin_html;
        }
    ...
    public function linkedin_get_comments(){
        ...
        if( is_array($comments) && !empty($comments) ) {
            foreach ( $comments as $comment ) {
               $html .= $this->template->block(
                                            '_comment',
                                            'twitter/activity/blocks/_one_linkedin_comment',
