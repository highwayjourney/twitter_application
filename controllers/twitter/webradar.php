...
class Webradar extends MY_Controller {
    ...
    public function index($social = null)
    {
        ...
        $this->template->current_view = 'twitter/webradar/index';
    ...
    private function getHtmlData($mentions)
    {
        ...
        if ($mentions->exists()) {
            ...
            foreach ($mentions as $mention) {
                ...
                $content = $this->template->block('_content', '/twitter/webradar/blocks/'.$mention->social, array('mention' => $mention));
                
                $blockData = array('mention' => $mention, 'content' => $content);
                $htmlData .= $this->load->view('twitter/webradar/blocks/_feed', $blockData, true);
            }
        }
