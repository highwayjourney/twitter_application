...
protected $website_part = 'dashboard';
    ...
    public function __construct()
    {
        ...
        CssJs::getInst()->c_js('twitter/create', 'product_search');
    ...
    public function get_posts(){
        if ($this->template->is_ajax()) {
			...
			if(!empty($post['source'])){   
                if ($this->template->is_ajax() && !empty($this->input->post())) {
                    $post = $this->input->post();
                    $search = array('keyword' => $post['keyword'] , 'category' => '', 'page' => $post['page']);
                    $affiliate_model = new affiliates_model();
                    $settings = $affiliate_model->get_affiliate_info($this->c_user->id, $this->profile->id);
                    $source = $this->load->library($post['source'] , $settings[$post['source'] ]);
                    try {
                        ...
                        echo json_encode($results);
                          ...
                          echo json_encode(array(
                            'success' => false,
                            'message' => 'Error.  Please check your Affiliate API Credentials'
                        ));   
                        exit();                        
                    }
                }                
