...
class Parallax extends MY_Controller
{
    ...
    public function __construct()
    {
        ...
        JsSettings::instance()->add(
            array(
                ...
                'pinterestLimits' => array(
                    'maxLength' => 500,
                    
                ),                
                'linkedinLimitsText' => lang('linkedin_error'),
                'pinterestLimitsText' => lang('pinterest_error')
            )
        );
        ...
        CssJs::getInst()->c_js('engage/parallax', 'social_limiter');
        CssJs::getInst()->c_js('engage/parallax', 'schedule_block');
        // CssJs::getInst()->c_js('parallax/create', 'app');
        // CssJs::getInst()->c_js('parallax/create', 'social_sources');
        // CssJs::getInst()->c_js('parallax/create', 'giphy');
        // CssJs::getInst()->c_js('parallax/create', 'facebook');
        // CssJs::getInst()->c_js('parallax/create', 'youtube');
    }

    public function index()
    {
        //$this->redirect('/engage/parallax');
    ...
    public function post_create()
    {
        if ($this->template->is_ajax()) {
            ...
            //var_dump($this->c_user->id, $this->profile->getTokenByTypeAsArray('facebook')['id']); die();
            if(in_array('pinterest', $post['post_to_socials'])) {
                $pinterest = Pinterest_board::inst()->get_selected_board($this->c_user->id, $this->profile->getTokenByTypeAsArray('facebook')['id']);
                if(!$pinterest) {
                    echo json_encode(array(
                        'success' => false,
                        'message' => lang('pinterest_board_error')
                    ));
                    exit();
                }
            }            
    ...
    public function search()
    {
        if ($this->template->is_ajax()) {
            ...
            if($post['source'] == 'facebook'){
                ...
                $page_feed = $facebook->get_page_by_keyword($post['keyword'], str_replace('https://graph.facebook.com/v2.6/', '', $post['since'] ));
                //var_dump($page_feed);
                echo(json_encode($page_feed));
            }                               
        }
    }
    public function get_posts(){
        if ($this->template->is_ajax()) {
            ...
            if($post['source'] == 'facebook'){
                ...
                $page_feed = $facebook->get_custom_page_posts($post['since']  , $post['object_id']);
                echo $page_feed;               
            }
            if($post['source'] == 'giphy'){
                $this->load->library('giphy/giphy');

                if($post['mode'] == 'search'){
                    $giphy = new Giphy();                    
                    if(is_numeric($post['keyword'])){
                        $posts = $giphy->getByID($post['keyword']);
                    } else {
                        $posts = $giphy->search($post['keyword'], 6, $post['since']);
                    }
                }                
                if($post['mode'] == 'trending'){
                    $giphy = new Giphy();
                    $posts = $giphy->trending(6);
                }
                if($post['mode'] == 'translate'){
                    $giphy = new Giphy();
                    $posts = $giphy->translate($post['keyword']);
                }                            
    ...
    public function post_rss()
    {
        ...
        JsSettings::instance()->add('parallaxDefaultType', 'midLength');
    ...
    {
        redirect('parallax/create/update');
    }
...
//            }
        ...
        if ($this->template->is_ajax()) {
            ...
            if ($user_rss->result_count() > 0) {
                echo $this->template->block(
                    '_suggested_links',
                    'parallax/create/blocks/_custom_rss',
                    array('rss' => $user_rss)
                );
            } else {
        ...
        if ($this->template->is_ajax()) {
            ...
            if (isset($post['link'])) {
                ...
                $html = $this->template->block(
                    '_rss_feed',
                    'parallax/create/blocks/_rss_feed',
    ...
    public function getRssByFeedId($feedId)
    {
            ...
            if ($feedId) {
                ...
                $html = $this->template->block(
                    '_rss_feed',
                    'parallax/create/blocks/_rss_feeder',
        ...
        if ($this->template->is_ajax()) {
            ...
            if (empty($errors)) {
                ...
                try {
                    ...
                    $result['errors'] = '<span class="message-error">' . $e->getMessage() . '</span>';
                }
    ...
    private function _check_socials_access() {
      ...
      private function _attach_update_scripts() {
        ...
        CssJs::getInst()->add_js(array(
            ...
            'libs/fabric/b64.js',
            'libs/fabric/LZWEncoder.js',
            'libs/fabric/NeuQuant.js',
            'libs/fabric/GIFEncoder.js',
            'libs/color/tinycolor-0.9.15.min.js',
            'libs/color/pick-a-color-1.2.3.min.js'
        ));
        CssJs::getInst()->c_js('engage/parallax', 'post_update');
        CssJs::getInst()->c_js('engage/parallax', 'post_attachment');
        CssJs::getInst()->c_js('engage/parallax', 'b64');
        CssJs::getInst()->c_js('engage/parallax', 'LZWEncoder');
        CssJs::getInst()->c_js('engage/parallax', 'NeuQuant');
        CssJs::getInst()->c_js('engage/parallax', 'GIFEncoder');
        CssJs::getInst()->c_js('engage/parallax', 'post_cron');
        CssJs::getInst()->c_js('engage/parallax', 'bulk_upload');
        CssJs::getInst()->c_js('engage/parallax', 'social_limiter');
        CssJs::getInst()->c_js('engage/parallax', 'schedule_block');
    }
