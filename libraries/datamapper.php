...
class DataMapper implements IteratorAggregate {
	...
	public function __call($method, $arguments)
	{
        ...
        switch (true) {
            case (0 === strpos($method, 'findBy')):
                $by = decamelize(substr($method, 6));
                $this->clear();
                return $this->where($by, $arguments[0])->get();
            case (0 === strpos($method, 'findOneBy')):
                $by = decamelize(substr($method, 9));
                $this->clear();
                return $this->where($by, $arguments[0])->get(1);
        }
	 ...
	 *
	 * @param	object|array $field A field to update, or an array of fields => values
    ...
    {
        return call_user_func_array(array(static::create(), $method), $arguments);
    }
}
