...
class Access_token extends DataMapper {
    ...
    /**
     * Get all social tokens  by user id and profile
     *
     * @param string  $type
     * @param integer $user_id
     * @param integer $profile_id
     ...
     * @return Access_token
     */
    static public function getAllByUserIdAndProfileIdAsArray($user_id, $profile_id)
    {
        $group = new Social_group($profile_id);
        $social_post = Social_post::$socials;
        foreach ($social_post as  $type) {
            $access_token[] = $group->access_token->get_array_by_type($type, $user_id, $profile_id);
        }
        return $access_token;
     ...
     * @return Access_token
     */
    public function add_token( $tokens = array(), $type, $user_id, $profile_id = null ) {
        ...
        $current_token = $this->getByTypeAndUserIdAndProfileId($type, $user_id, $profile_id);
        //}          
    ...
    public static function getGroupTokensArray($group_id, $exclude = array(), $has_twitter_marketing_tools = false) {
        ...
        foreach (self::$types as $type) {
            ...
            $has_configs = (count(Available_config::getByTypeAsArray($type)) || ($type == 'facebook') || ($type == 'pinterest'))
                ? true
                : false;
