...
class Job_queue extends DataMapper {
     ...
     * @param int $lifeTime time of job's actuality    
     */
    public function dbMantain(){
        //Added for DB Sanity 
        $limit = new DateTime('-300 sec');
        $result = $this->where('state =', 3)
                        ->where('started_at IS NOT null')
                        ->where('started_at <', $limit->format($this->dateFormat))->get();
            //ddd($result->all_to_array());
            foreach($result as $job) {
               $job->delete();
            }             
    }
    /**
     * Check if there are unfinished jobs by thread
     *  
     */    
    public function checkJob($thread){
        $result = $this->where('state =', 0)
                 ->where('thread = ', $thread)->count();
        if($result > 0 ){
            return true;
        } else {
            return false;
