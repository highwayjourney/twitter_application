...
class Social_post_cron extends DataMapper {
    ...
    public static function add_new_post($feeds, $user_id, $profile_id) {
        ...
        $post->setTimeInUtc($feeds['cron_schedule_time'], $feeds['cron_day'], $feeds['timezone']);
     ...
     */
    public function setTimeInUtc($time_in_utc, $day, $timezone = null) {
        if($timezone) {
            $_timezone = new DateTimeZone($timezone);
            $_utc_timezone = new DateTimeZone('UTC');
            foreach($time_in_utc as $key => &$time) {
                $date = new DateTime($day[$key]." ". $time, $_timezone);
                $date->setTimezone($_utc_timezone);
                if($_SERVER['REMOTE_ADDR'] == '186.89.150.214'){
                    $time = $date->format('D h:i A');
                } else {
