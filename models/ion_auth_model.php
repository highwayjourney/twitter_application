...
class Ion_auth_model extends CI_Model
{
     ...
     * @author Mathew
     */
	public function register($username, $password, $email, $additional_data = array(), $groups = array(), $throught_social=FALSE)
	{
        ...
        $registered = (isset($id)) ? $id : FALSE;
        if($registered) {
            $profile = new Social_group();
            $profile->user_id = $id;
     ...
     * @return bool
     */
    public function registerByInvite($params)
    {
        if ($params['id'] && $params['password']) {
            $user = new User($params['id']);
            if ($user->active != $user::STATUS_INVITE) {
                return false;
            }
            $ip_address = $this->_prepare_ip($this->input->ip_address());
            $salt       = $this->store_salt ? $this->salt() : FALSE;
    ...
    public function getGroupByName($name)
    {
        ...
        if (isset($name))
        {
            $this->db->where($this->tables['groups'].'.name', $name);
        }
     ...
     * @return mixed
     **/
    public function getUsersByGroup($name)
    {
        $id = $this->getGroupByName($name)->id;
        $this->trigger_events('get_users_group');

        $result =  $this->db->select($this->tables['users_groups'].'.'.$this->join['users'])
     ...
     * @param      $password
     * @param bool $remember
     * @param bool $throught_social
     *
     * @return bool
     * @author Mathew
     */
	public function login($identity, $password, $remember=FALSE, $throught_social=false)
	{
		$this->trigger_events('pre_login');

		if (!$throught_social && (empty($identity) || empty($password)))
		{
		...
		if ($query->num_rows() === 1)
		{
			...
			if ($password === TRUE || $throught_social)
			{
				if ($user->active == 0)
				{
					$this->trigger_events('post_login_unsuccessful');
     ...
     * @return bool
     */
    public function loginForce($identity, $remember=FALSE)
    {
        $this->trigger_events('pre_login');
    ...
    public function getActiveUser($id = NULL)
    {
        ...
        $user = new User($id);
        if ($this->ion_auth->is_collaborator($id)) {
            $group = $this->ion_auth_model->getGroupByName($this->config->item('default_group', 'ion_auth'));
            $user = $user->getMajorUserInGroup($group->id);
        }
        $result = ($user->exists()) ? $user : null;
	...
	**/
	...
	public function remember_user($id)
	{
		...
		if (!empty($user))
		{
            ...
            $this->session->migrate(false, $expire);
