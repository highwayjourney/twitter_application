...
class MY_Controller extends Base_Controller
{
    ...
    public function __construct($website_part = null)
    {
        ...
        foreach ($_plans as $key => $value) {
            ...
            if($_plan['id'] != 6 ){
                $planes[$_plan['id']] = $_plan['name'];
            }    
        }

        if(!empty($this->c_user) && !empty($planes)){
            $this->template->set('planes', $planes);
            if(!empty($this->c_user->getActivePlan())){
                $this->template->set('current_plan', $this->c_user->getActivePlan()->to_array()['id']);
            }
        }
        //ddd($plans, $this->c_user->getActivePlan()->to_array()['id']);
...
//ddd($this->paymentsEnabled , $this->trialEnabled);
        if($this->router->fetch_method() !== 'updateLanguage') {
            ...
            if (!$this->ion_auth->logged_in()
                ...
                && $this->router->fetch_class() !== 'terms'
                //&& $this->router->fetch_class() !== 'trivia'
            ) {
                redirect('auth');
            }
            if ($this->ion_auth->logged_in()
                ...
                && $this->router->fetch_method() !== 'logout') 
            {
                ...
                // var_dump($social_group);
    //            var_dump($social_group->access_token);
    //            exit();
                
                $this->template->set('user_image', empty($args['image'])?null:$args['image']);
